...
function ChatModal({ visibleModal, setVisibleModal, shipment, user, auth, conversation, getConversation, setAuth }) {
    ...
    async function sendNewMessage() {
        try {
            ...
            if (response.status === 401) {
                ...
                if (newToken) {
                    response = await fetch(`${API_URL}/api/shared/conversations/messages?conversationId=${conversation.conv_id}`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
    ...
    async function createNewConversation() {
        try {
            ...
            if (response.status === 401) {
                let newToken = await refreshToken(setAuth, navigate);
                if (newToken) {
                    response = await fetch(`${API_URL}/api/shared/conversations?shipmentNumber=${shipment.shipment_number}`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'Authorization': `Bearer ${newToken}`
